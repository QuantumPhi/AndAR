...
public class Synthesiser {
	...
	public InputStream getMP3Data(List<String> synthText) throws IOException{
		...
		Set<AsyncTask<String, Void, InputStream>> set = new LinkedHashSet<AsyncTask<String, Void, InputStream>>(synthText.size());
		for(String part : synthText) //Iterates through the list
			set.add(new MP3DataFetcher(part)); //Adds the response that will be returned to a set.
		List<InputStream> inputStreams = new ArrayList<InputStream>(set.size());
        InputStream value;
		for(AsyncTask<String, Void, InputStream> future : set){
			try {
                value = future.get();
                if(value == null)
                    throw new IOException();
				inputStreams.add(value); //Gets the returned data from the future.
	 ...
	 */
	private class MP3DataFetcher extends AsyncTask<String, Void, InputStream>{
